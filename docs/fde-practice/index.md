...
nav_label: FDE実践大全
description: 顧客の現場に入り込み、課題発見から本番運用の定着までを引き受けるフォワードデプロイエンジニアの技術を、六部14章でまとめた実践書。データ基盤、RAGとエージェント、API設計、評価とLLMOps、そして引き継ぎまでを、現場の制約とトレードオフから解説する。
status: 本文あり（ドラフト）
order: 3
tags:
  - FDE
...
| 付録 | 構造化データと文書をまたぐ検索の実装例、用語集 |

第I部と第VI部は職能論、第II部から第V部は技術論である。すでに現場に入っている読者は第3章から読み始め、必要になったときに第1章へ戻る読み方でも筋は通る。付録Aは第4章と第6章の内容を一つの流れにつないだ実装例なので、両章を読んだあとに読むと効く。用語の初出章は付録Bで引ける。
