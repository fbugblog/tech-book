...
## 13.4 モデルの差し替えに耐える設計 {#model-migration}

LLMアプリケーションが従来のシステムと決定的に違う点の一つは、**土台が数か月単位で入れ替わること**である。プロバイダーは新しいモデルを出し、古いモデルには提供終了の期限が付く。顧客の稟議が一年かかる環境でも、モデルの世代交代はそれを待たない。

したがって、差し替えを例外イベントではなく**定期的に起きる運用作業**として設計する。

### 13.4.1 差し替えの影響が出る箇所

同じプロンプトを新しいモデルに渡しても、挙動は変わる。実務で影響が出やすいのは次の箇所である。

| 影響箇所 | 起きること | 検出方法 |
| --- | --- | --- |
| 出力の形式 | スキーマ検証の失敗率が変わる（多くは改善するが、稀に悪化する） | `status` 別のエラー率（13.1） |
| 冗長さ | 出力トークン数が増減し、費用と体感が変わる | トークン数の分布 |
| 棄権の傾向 | 慎重になって答えなくなる、または逆に答えすぎる | 棄権率（第12章） |
| ツール選択 | 呼ぶツールの順序や引数の埋め方が変わる | 経路の妥当性（第8章） |
| 日本語の語調 | 敬体の揺れ、専門用語の言い換え | 現場レビュー |
| プロンプトの効き方 | 強調していた指示の効果が変わる | 評価データセット全体 |

最後の行が最も厄介である。「絶対に計算しないこと」という指示が効いていたモデルから、その指示の優先度が下がるモデルに移ると、第6章で設計した安全装置が静かに緩む。**プロンプトの指示は、モデルごとに効き方が違う前提で扱う。**

### 13.4.2 段階的に切り替える

一斉切り替えは避ける。評価と本番のトラフィックを使って、段階的に確認する。

```mermaid
flowchart TB
    S1["段階1: オフライン評価<br/>評価データセット全件を新旧で実行"] --> D1{指標が同等以上か}
    D1 -->|劣化| FIX["プロンプトを新モデル向けに調整<br/>（旧モデル用とは別バージョンとして持つ）"]
    FIX --> S1
    D1 -->|同等以上| S2["段階2: 影武者実行<br/>本番の入力を新モデルにも流し、<br/>出力は利用者に見せず記録だけする"]
    S2 --> D2{差分の傾向を確認}
    D2 --> S3["段階3: 部分切り替え<br/>影響の小さい用途から10%→50%"]
    S3 --> D3{指標・費用・苦情の推移}
    D3 -->|問題あり| RB[即時に旧モデルへ戻す]
    D3 -->|問題なし| S4["段階4: 全面切り替え<br/>旧モデルの経路は一定期間残す"]
```

段階2の**影武者実行（シャドー実行）**は、費用が二重にかかるため期間を限定する。それでも価値がある。評価データセットには現れない実際の入力——利用者特有の言い回し、想定外の長文、添付の混在——に対する挙動の差が、ここで初めて見える。

実装上は、モデルの指定を設定として外に出しておくことが前提になる。第7章でプロンプトをモデル設定とともにファイルで管理したのは、この差し替えを設定変更として行えるようにするためである。

```yaml
# config/models.yaml — 用途ごとに指定し、切り替えを設定変更として扱う
routes:
  invoice_review:
    primary: { provider: anthropic, name: claude-sonnet-5, prompt: invoice_review@3 }
    shadow:  { provider: anthropic, name: claude-opus-5,   prompt: invoice_review@4, sample_rate: 0.1 }
    fallback: { provider: anthropic, name: claude-haiku-4-5, prompt: invoice_review@3 }
  classification:
    primary: { provider: anthropic, name: claude-haiku-4-5, prompt: classify@2 }
```

`fallback` を持たせておくと、主系が障害やレート制限で使えないときに処理を継続できる。ただし**フォールバックで品質が落ちることを利用者に伝える**設計にする。黙って劣化したモデルで応答を続けると、その期間の出力が業務に混ざり、後から区別できない。トレース（13.1）に実際に使ったモデル名を記録しておけば、後追いで特定できる。

### 13.4.3 提供終了への備え

モデルの提供終了は、事前に告知されるが猶予は長くない。備えとして三つを準備しておく。

- **使用しているモデルの一覧を機械的に出せる状態にする。** 設定ファイルを走査すれば列挙できるようにしておく。コードに直書きされていると棚卸しができない
- **切り替え手順を一度実際に流しておく。** 第3章でインデックス再構築について述べたのと同じ理由である。一度も実行していない手順は、実行できない手順と同じだ
- **評価データセットを常に動く状態に保つ。** 提供終了の期限が迫った状況で、評価の仕組みから作り始めるのは間に合わない

::: warning 顧客への説明が必要になる
モデルの差し替えは、顧客にとっては「勝手にシステムの中身が変わる」ことを意味する。稟議や監査の観点から、変更管理の対象として扱うべき環境も多い。FDEとしては、導入時に**モデルは更新されるものであり、更新時には評価をもって同等性を確認する**という運用を先に合意しておく。後から説明すると、変更そのものが承認プロセスで止まる。
:::
...
モデルの差し替えは例外ではなく定期的な運用作業である。オフライン評価、影武者実行、部分切り替え、全面切り替えという四段階で進め、モデルの指定は設定として外に出しておく。
...
- モデルの差し替えは定期的な運用作業として設計する。プロンプトの指示の効き方はモデルごとに違う
- 切り替えは四段階で進め、影武者実行で評価データセットに現れない入力への挙動差を見る
- フォールバックで品質が落ちることは利用者に伝え、実際に使ったモデル名をトレースに記録する
:::
