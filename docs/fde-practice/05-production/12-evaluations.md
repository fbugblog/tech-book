...
## 12.5 評価の運用——誰が正解を決めるか {#eval-ownership}

ここまでは技術の話だった。実務で評価が止まる原因は、ほとんど技術ではなく**運用の責任が決まっていないこと**にある。

### 12.5.1 正解を決める権限を明示する

評価データセットの「期待される答え」は、誰かが決めなければならない。ここを曖昧にしたまま進めると、次の事態が起きる。開発者が自分の判断で正解を作り、それにもとづいて指標が改善し、そして本番で業務担当者から「この回答は不適切だ」と指摘される。指標は嘘をついていないが、測っていた対象が業務の要求とずれていた。

避けるには、ケースの種別ごとに**正解の決定者**を決めておく。

| 種別 | 正解を決める人 | 根拠 |
| --- | --- | --- |
| 規程の解釈 | その規程の管掌部門 | 解釈権限が業務上そこにある |
| 数値の集計 | データの所管部門 | 指標定義の責任者（第4章のセマンティックレイヤー） |
| 回答すべきか否か | 情報統制の担当 | 開示範囲の判断は権限管理の問題 |
| 文面の適切さ | 実際に利用する現場 | 過剰に丁寧、または冷たいという感覚は現場が持つ |

この表の効用は、意見が割れたときの決着先が決まることにある。「規程の解釈で開発者と現場の意見が違う」という状況で、管掌部門に聞けばよいと事前に決まっていれば、議論は数日で終わる。決まっていないと、担当者の力関係で決まり、そして後から蒸し返される。

### 12.5.2 評価の頻度と担当を決める

評価は誰かの業務時間を消費する。無償の善意に頼ると続かない。

```mermaid
flowchart LR
    subgraph auto[自動・毎回]
        A1[決定論チェック<br/>CI・開発者]
        A2[判定器による採点<br/>CI・開発者]
    end
    subgraph weekly[週次・30分]
        W1[新規の失敗ケースを確認<br/>FDE＋業務担当1名]
        W2[却下理由の分類集計]
    end
    subgraph monthly[月次・60分]
        M1[指標の推移レビュー<br/>業務責任者を含む]
        M2[データセットへの追加を決定]
    end
    auto --> weekly --> monthly
    monthly -.->|判断基準の更新| auto
```

この図が主張しているのは、**人間が関わる工程を時間で区切って定例化する**ことである。週30分と月60分。これ以上を求めると、二か月目に出席率が落ちる。少なく見えるが、自動化された部分が毎回走っていれば、人間は差分だけを見ればよい。

FDEが去った後もこのサイクルが回るかどうかが、第14章の引き継ぎの成否を分ける最初の試験になる。

### 12.5.3 評価データセットが腐る三つの形

最後に、運用していく上で必ず起きる劣化を挙げる。

**業務が変わって正解が変わる。** 規程が改定され、昨年の正解が今年は誤りになる。対策は、各ケースに**根拠となる文書の版**を持たせること（第6章のチャンクに `version` を持たせたのと同じ理由）。文書が改定されたら、その文書を根拠とするケースに再確認のフラグを立てる。

```python
class EvalCase(BaseModel):
    # （前略）
    basis_doc_ids: list[str] = []        # このケースの正解が依拠する文書
    basis_version: str | None = None     # その版
    needs_review: bool = False           # 文書改定で自動的に立てる
    last_confirmed_at: date | None = None
```

**過学習する。** 同じ60件を見ながらプロンプトを調整し続けると、その60件だけで良い成績が出るようになる。対策は、**評価用と検証用を分けること**。ケースの三割を検証用として取り分け、日常の改善では見ない。月次のレビューでのみ確認する。

**簡単なケースだけが増える。** 運用で追加されるケースは、担当者が気づいた失敗、つまり明らかな失敗に偏る。難しいケース、判断が微妙なケースは増えにくい。対策は、追加時に12.1.2の比率を意識し、「答えられない質問」と「紛らわしい質問」の枠を意図的に埋めることである。

> 評価データセットは資産だが、放置すれば負債になる。腐り方を知っていれば、腐る前に手を入れられる。
...
そして運用では、ケースの種別ごとに正解の決定者を決め、人間が関わる工程を週30分・月60分として定例化する。評価データセットは、業務変更・過学習・簡単なケースへの偏りという三つの形で腐るため、文書の版を持たせ、検証用を取り分け、追加時の比率を意識する。
...
- 正解の決定者をケースの種別ごとに決めておく。意見が割れたときの決着先が事前に決まる
- 人間が関わる工程は週30分・月60分で定例化する。それ以上を求めると二か月目に続かなくなる
- データセットの腐り方は三つ。業務変更に備えて根拠文書の版を持ち、検証用を取り分け、難しいケースの枠を埋める
:::
